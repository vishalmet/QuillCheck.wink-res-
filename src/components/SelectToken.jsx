...
const SelectToken = ({ onCheckClick, setSelectedToken, setTokenAddress, setChainId, empty, setempty , buttonclick , setButtonclick }) => {
  ...
  return (
    <div
      className="bg-[#18162099]/60 rounded-[10px] backdrop-filter backdrop-blur-sm  w-[460px] sm:w-fit mx-auto p-[10px] sm:p-[30px] jost"
      ...
      <div className="mt-2 sm:mt-4 space-y-2 sm:space-y-4">
        <div className="flex justify-center space-x-1 sm:space-x-3">
          {['ETH', 'BSC', 'POL', 'Base', 'SOL'].map((token, index) => (
            ...
            >
              <img className="h-4 w-auto" src={Assets[token]} alt={token} /><span className='flex'>{token}</span>
            </button>
          ))}
        </div>
        <input
          ...
          className="bg-white w-[400px] sm:w-full h-8 text-xs sm:text-base sm:h-12 rounded-[5px] text-black p-2 sm:p-4"
